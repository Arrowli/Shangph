        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="saleList in spuSaleAttrList" :key="saleList.id">
                <dt class="title">
                  {{ saleList.saleAttrName }}
                </dt>
                <dd
                  changepirce="0"
                  :class="{ active: valueList.isChecked == 1 }"
                  v-for="valueList in saleList.spuSaleAttrValueList"
                  :key="valueList.id"
                  @click="
                    changeActive(valueList, saleList.spuSaleAttrValueList, skuInfo)
                  "
                >
...
export default {
  ...
  methods: {
    changeActive(valueList, arr, skuInfo) {
      arr.forEach((item) => {
      ...
      //skuInfo里的属性也应该变
      skuInfo.skuSaleAttrValueList.forEach((item)=>{
        if(item.saleAttrName == valueList.saleAttrName){
          item.saleAttrValueName = valueList.saleAttrValueName
        }
      })
      // console.log('test', valueList.saleAttrValueName)
    },
